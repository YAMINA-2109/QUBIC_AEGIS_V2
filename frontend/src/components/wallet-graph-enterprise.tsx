...
const INITIAL_DEMO_DATA = {
  nodes: [
    ...
    { id: "Victim-1", label: "Victim 1", value: 50000, group: "normal", risk_score: 20 },
    { id: "Victim-2", label: "Victim 2", value: 30000, group: "normal", risk_score: 25 },
    {
      id: "Unknown-Attacker",
      label: "Attacker",
      value: 3000000,
        ...
        if (data.nodes && data.nodes.length > 0) {
          ...
          if (data.edges) {
            processedNodes = data.nodes.map((node: ApiNode, idx: number) => ({
              id: node.id || `node-${idx}`,
              label: node.label || node.id?.substring(0, 12) || `Wallet ${idx + 1}`,
            ...
            processedLinks = data.links.map((link: ApiLink) => ({
              source: typeof link.source === "number" ? `node-${link.source}` : link.source,
              target: typeof link.target === "number" ? `node-${link.target}` : link.target,
              value: link.value || 1,
            }));
          }
          ...
          if (filterRisk !== "all") {
            const filteredNodes = processedNodes.filter((node) =>
              filterRisk === "high" ? node.risk_score && node.risk_score > 70 : node.risk_score && node.risk_score <= 70
            );
          ...
          if (data.clusters && data.clusters.length > 0) {
            console.log(`Detected ${data.clusters.length} clusters:`, data.clusters);
          }
        }
      } catch (error) {
      ...
      <CardContent>
        <div className="h-[600px] rounded-lg border-2 border-primary/20 bg-gradient-to-br from-black via-gray-950 to-black relative overflow-hidden">
          ...
          <div className="absolute top-4 left-4 z-10 bg-black/90 backdrop-blur-sm p-3 border border-primary/40 rounded-md shadow-lg">
            <div className="text-xs font-mono text-primary mb-2">NETWORK STATS</div>
            <div className="space-y-1 text-xs text-foreground">
              <div>Nodes: <span className="text-primary font-bold">{graphData.nodes.length}</span></div>
              <div>Connections: <span className="text-primary font-bold">{graphData.links.length}</span></div>
              <div>High Risk: <span className="text-destructive font-bold">
                {graphData.nodes.filter(n => (n.risk_score || 0) > 70).length}
              </span></div>
            </div>
          </div>
          ...
          {selectedNode && (
            <div className="absolute top-4 right-4 z-10 bg-black/90 backdrop-blur-sm p-3 border border-primary/40 rounded-md shadow-lg max-w-xs">
              <div className="text-xs font-mono text-primary mb-2">WALLET DETAILS</div>
              <div className="space-y-1 text-xs text-foreground">
                <div className="font-bold text-primary break-all">{selectedNode.label}</div>
                <div>ID: <span className="text-muted-foreground font-mono text-[10px]">{selectedNode.id}</span></div>
                <div>Risk: <span className={`font-bold ${(selectedNode.risk_score || 0) > 70 ? 'text-destructive' : 'text-primary'}`}>
                  {(selectedNode.risk_score || 0).toFixed(1)}%
                </span></div>
                <div>Volume: <span className="text-primary font-mono">{(selectedNode.value || 0).toLocaleString()}</span></div>
                <div>Type: <span className="uppercase text-muted-foreground">{selectedNode.group}</span></div>
              </div>
            </div>
          )}
          ...
          <ForceGraph2D
            ...
            nodeCanvasObject={(node: GraphNode, ctx: CanvasRenderingContext2D, globalScale: number) => {
              ...
              ctx.fillText(label, node.x || 0, (node.y || 0) + (getNodeSize(node) * 0.8));
