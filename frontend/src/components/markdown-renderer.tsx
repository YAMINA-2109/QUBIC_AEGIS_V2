  ...
  const parseMarkdown = (text: string): string => {
    ...
    html = html.replace(/```(\w+)?\n?([\s\S]*?)```/g, (_match, _lang, code) => {
      ...
      codeBlockPlaceholders.push(
        `<pre class="bg-black/80 p-4 rounded border border-gray-700 my-4 overflow-x-auto"><code class="text-[#00ff41] text-sm font-mono whitespace-pre">${code.trim()}</code></pre>`
      );
      return id;
    });
    ...
    html = html.replace(/`([^`\n]+)`/g, (_match, code) => {
      ...
      inlineCodePlaceholders.push(
        `<code class="bg-black/60 px-1.5 py-0.5 rounded text-[#00ff41] text-sm font-mono">${code}</code>`
      );
      return id;
    });
    ...
    html = html.replace(
      /^#### (.*)$/gm,
      '<h4 class="text-base font-bold text-[#00ff41] mt-5 mb-2 font-mono">$1</h4>'
    );
    html = html.replace(
      /^### (.*)$/gm,
      '<h3 class="text-lg font-bold text-[#00ff41] mt-6 mb-3 font-mono">$1</h3>'
    );
    html = html.replace(
      /^## (.*)$/gm,
      '<h2 class="text-xl font-bold text-[#00ff41] mt-8 mb-4 font-mono">$1</h2>'
    );
    html = html.replace(
      /^# (.*)$/gm,
      '<h1 class="text-2xl font-bold text-[#00ff41] mt-10 mb-5 font-mono">$1</h1>'
    );
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (unorderedMatch || orderedMatch) {
        ...
        if (currentList.length === 0) {
          ...
          currentList.push(
            unorderedMatch ? unorderedMatch[1] : orderedMatch![2]
          );
          ...
          processedLines.push(
            `<${listTag} class="my-3 ml-6 space-y-2 ${
              isOrderedList ? "list-decimal" : "list-disc"
            }">${currentList
              .map((item) => `<li class="text-gray-300">${item}</li>`)
              .join("")}</${listTag}>`
          );
    ...
    html = html.replace(
      /\[([^\]]+)\]\(([^\)]+)\)/g,
      '<a href="$2" class="text-[#00ff41] hover:underline" target="_blank" rel="noopener noreferrer">$1</a>'
    );
